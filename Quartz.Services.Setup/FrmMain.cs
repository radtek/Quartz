...
namespace WSWinForm
{
    public partial class FrmMain : AlSkin.AlForm.AlBaseForm
    {
        ...
        private string GetServiceName(string serviceFileName, Boolean Descriptions = false)
        {
            try
            {
                    ...
                    if (myType.IsClass && myType.BaseType == typeof(System.Configuration.Install.Installer))
                    {
                        ...
                        foreach (FieldInfo myFieldInfo in fieldInfos)
                        {
                            if (myFieldInfo.FieldType == typeof(System.ServiceProcess.ServiceInstaller))
                            {
                                ...
                                Boolean DelayedAutoStart = true;

                                if (Descriptions)
                                {
                                    DisplayName = serviceInstaller.DisplayName;
                                    Description = serviceInstaller.Description;
                                    ServiceStartMode = serviceInstaller.StartType;
                                    DelayedAutoStart = serviceInstaller.DelayedAutoStart;

                                    //InstallContext Context = serviceInstaller.Context;
                                ...
                                return !Descriptions
                                    ? ServiceName
                                    : "\r\n当前服务：\t" + ServiceName + Environment.NewLine +
                                        "服务名称：" + DisplayName + Environment.NewLine +
                                        "服务说明：" + Description + Environment.NewLine +
                                        "启动模式：" + ServiceStartMode.ToString() + Environment.NewLine +
                                        "自动启动：" + DelayedAutoStart + Environment.NewLine + Environment.NewLine +
                                        "--------------------------------------------------------" + Environment.NewLine +
                                        Environment.NewLine + Environment.NewLine +
                                        "程序开发：JosonJiang -> " + DateTime.UtcNow + Environment.NewLine;
                            }
                        }
                    }
                }
        ...
        private void btnInstall_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    txtTip.Text = "要安装的服务已经存在！\n\r\r\n" + GetServiceName(serviceFileName, true);
                    return;
                }
